...
### An example of creating and using a UDF
...
Here, we provide a schema to `in_type` so that we can map function argument names to types, and we give a data type to `out_type`.

The `auto_convert` argument controls whether arrow automatically converts the inputs to R vectors or not.
The default value, `FALSE`, means that inputs to your R function will be Arrow objects.
In our example, this would mean that `x` would come in as an Arrow Array of type `string()`.
But we want an R vector for `x` so that we can pass it to the stringr function.
We could call `as.vector()` on `x` to convert the Array into an R vector, or we can do what we've done in this example and set `auto_convert = TRUE` to handle that for us.
Setting `auto_convert = TRUE` also handles the return value conversion back to Arrow data for us.
...
### Using UDFs for regression model predictions

Another example where UDFs can be useful is where you've created a model based on a subset of your data and you want to use it to make predictions.
Let's say we've created a linear model, using education and employment status to predict hours worked per week for respondents in 2018.
When creating this model we aren't using the standard `stats::lm()` because our data is weighted. 
The `survey` package has an equivalent regression function `survey::svyglm()` which applies the weights in the weight column during the regression. 
This isn't strictly necesary for the demonstration here, but it does show how UDFs work not just with base R functions or functions in the tidyverse, but any R code that you come up with.
...
We can use this model to predict what our model would have predicted on this data if it were new data coming in.
This is a little artificial since this is the same data that we used to build the model, though it's a good check to confirm that the modeling process doesn't have anything terribly off about it.
Also because we are predicting on the same data we fit this linear model on, this is going to be very close to the actual weighted means.
We use the `predict` function to make these predictions and we also calculate the weighted mean for each group.

```{r}
#| label: summarise_fit_data
#| warning: false
...
And we can plot our model as well as the predictions to visualize what is going on.
Here the underlying data is represented in the density curve --- the taller the curve the more observations are at that point on the x-axis (hours worked per week). 
At the bottom the line represents the weighted mean and the diamond is the prediction from our model.
And like we said above, we can confirm that the weighted means and the model predictions are incredibly close.[^they-better-be]

[^they-better-be]: They better be very close since this is the same data the linear regression was fit with!
...
plot <- ggplot(model_data) +
  ...
  geom_violinhalf(scale = "width", fill = "grey", color = "grey") +
  layer(
    geom = "point",
    stat = "identity",
    position = position_nudge(x = -0.08),
    data = model_data_summary,
    mapping = aes(x = edu, y = work_hours_mean, weight = NULL),
    params = list(shape = 108, size = 6)
  ) +
  layer(
    geom = "point",
    stat = "identity",
    position = position_nudge(x = -0.08),
    data = model_data_summary,
    mapping = aes(x = edu, y = work_hours_predicted, weight = NULL),
    params = list(shape = 18, size = 4)
  ) +
  facet_grid(
    cols = vars(self_employed),
    labeller = as_labeller(c("TRUE" = "self-employed", "FALSE" = "not self-employed"))
  ) +
  coord_flip() +
  theme_minimal() +
  labs(
    title = "Hours worked per week",
    subtitle = "by employment type in 2018, all of US\nvertical line is weighted mean, diamond model prediction",
    x = "education",
...
```{r}
#| label: plot_fit_data_plot-pdf
#| echo: false
#| warning: false
#| fig-height: 9
#| fig-cap: "Hours worked per week by employment type in 2018 for all of the United States. The density curve is the raw data, the line at the bottom the weighted mean and the diamond at the bottom is the weighted mean of the model predictions."
plot
```
:::

Let's say we want to use this model to predict working hours in Alaska in 2021.
...
#| warning: false
#| echo: false
modelled_data_summary <- modelled_data |>
  group_by(edu, self_employed) |>
  summarise(
    work_hours_mean = weighted.mean(hours_worked, w = PWGTP, na.rm = TRUE),
    work_hours_predicted = weighted.mean(predicted, w = PWGTP),
...
plot <- ggplot(modelled_data) +
  ...
  theme_minimal() +
    labs(
      title = "Hours worked per week",
      subtitle = "by employment type in 2018, Alaska\nvertical line is weighted mean, diamond model prediciton",
      x = "education",
      y = "hours worked per week"
    )
```
...
::: {.content-visible when-format="pdf"}
```{r}
#| label: plot_predictions_plot-pdf
#| warning: false
#| echo: false
#| fig-height: 9
#| fig-cap: "Hours worked per week by employment type in 2021 for Alaska. The density curve is the raw data, the line at the bottom the weighted mean and the diamond at the bottom is the weighted mean of the model predictions."
plot
```
:::

We are slightly off in most categories, but especially far off for high school and some college educated workers where we predict much lower working hours.
Not totally surprising given our original model was built with all of the US and states differ considerably on this dimension.

Let's test that theory out: we can also re-use this UDF for any subset, including predicting on all of the US in 2021.

```{r}
#| label: predict-commute-time-all-us
modelled_data <- open_dataset("data/person/") |>
  filter(year == 2021) |>
  filter(AGEP >= 16, AGEP < 65, !is.na(COW)) |>
    transmute(
...
::: {.content-visible when-format="html"}
```{r}
#| label: plot_predictions_all_us_plot
#| echo: false
#| warning: false
plot
```
...
::: {.content-visible when-format="pdf"}
```{r}
#| label: plot_predictions_all_us_plot-pdf
#| echo: false
#| warning: false
#| fig-height: 9
#| fig-cap: "Hours worked per week by employment type in 2021 for all of the United States. The density curve is the raw data, the line at the bottom the weighted mean and the diamond at the bottom is the weighted mean of the model predictions."
plot
```
:::

And we can see the fit is slightly better, though we still slightly over predict, particularly for self-employed workers.
From here we might explore fitting a multi-level model with varying slopes or intercepts for each state or add more predictors to our model.
But no matter what we choose, we can use UDFs to do prediction like we did here.
...
We can see that this returns a data frame with one row per PUMA and language, with the number of speakers and the proportion of speakers whose primary language is not English compared to the overall population.
...
#| eval: false
puma_by_langauge |>
  filter(language %in% c("german", "spanish", "french", "navajo")) |>
  fill_pumas() |>
  inner_join(
    PUMA,
  ...
  labs(
    title = "Proportion of non-English speakers by language",
...
plot <- puma_by_langauge |>
  ...
  correct_projection() |>
  ggplot() +
  geom_sf(aes(geometry = geometry, fill = prop_speaker), lwd = 0, color = NA) +
  coord_sf(crs = "+proj=laea +lon_0=-98 +lat_0=39.5") +
  facet_wrap(vars(language), ncol = 2) +
...
#| echo: false
#| fig-height: 7
#| fig-cap: "Proportion of speakers whose primary language is not English by language across the US by PUMA for German, French, Navajo, and Spanish"
plot
```
:::

We can clearly see some patterns of migration: French is spoken in parts of northern Maine and Louisiana; German is spoken in the midwest, especially in the upper midwest and intermountain west.
...
#| eval: false
puma_by_langauge |>
  filter(language %in% c("chinese", "spanish", "polish")) |>
  fill_pumas() |>
...
#| echo: false
#| fig-height: 5
#| fig-cap: "Proportion of speakers whose primary language is not English by language in the Chicago by PUMA for Chinese, Polish, and Spanish"
plot +
  scale_x_continuous(
...
#| eval: false
puma_by_langauge |>
  filter(language %in% c("chinese", "spanish", "french",
                         "portuguese")) |>
  fill_pumas() |>
  inner_join(PUMA, by = c("year" = "YEAR",
                          "location" = "location",
...
#| echo: false
plot <- puma_by_langauge |>
  filter(language %in% c("chinese", "spanish", "french", "portuguese")) |>
  fill_pumas() |>
  inner_join(PUMA, by = c("year" = "YEAR", "location" = "location", "PUMA" = "PUMA")) |>
  correct_projection() |>
  ggplot() +
...
#| echo: false
#| fig-height: 9
#| fig-cap: "Proportion of speakers whose primary language is not English by language in the Northeast by PUMA for Chinese, French, Portuguese, and Spanish"
