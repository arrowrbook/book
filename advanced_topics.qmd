...
In Chapter [data manip whatever] we mentioned that the arrow R package transforms dplyr syntax into Arrow Expressions which can be understood by Acero, and here we're going to give into that a little deeper.  

Acero, arrow's query engine, has a large number of functions implemented for performing calculations on data.  The complete list can be found in the docs here: https://arrow.apache.org/docs/cpp/compute.html#available-functions
...
<!-- add screenshot of sd docs page and then screenshot of varianceoptions and stddev
https://arrow.apache.org/docs/cpp/api/compute.html#_CPPv4N5arrow7compute15VarianceOptionsE
https://arrow.apache.org/docs/cpp/compute.html#available-functions
-->
...
You don't have to worry about these differences yourself though, as the code in the arrow R package does a direct translation from one to the other.  Let's take a closer look at an example.

Let's say we wanted to only return data for states whose names start with the letter "w".  We can take our dataset and use `dplyr::filter()` to return this subset.
We need to pick a function to pass to `dplyr::filter()` to actually *do* the filtering.
