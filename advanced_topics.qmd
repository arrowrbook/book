...
* like taking the query and parsing it into an algebraic format
* re-ordering and optimising the query in order to run it in the most efficient way
* actually running the query

Acero doesn't contain a query optimizer, but there is some code in the R package which reorders queries before passing it to Acero.

When you call dplyr functions on Arrow tabular objects like tables and datasets, the code in the R package translates this into execution plan - directed graphs which express what operations need to take place.

These are represented by nodes.
...
open_dataset("data/person/") |>
  ...
  show_query()
```
For the most part, we don't have to worry too much about the details of these; though they can be useful for investigating queries which run slower than expected, to get a closer look at exactly what is being run in Acero.
...
The latter function is more commonly used as it accounts for overflow and raises an error if the value returned is beyond the minimum or maximum value for that data type.

You don't usually have to worry about these details yourself though, as the code in the arrow R package does a direct translation from R function to the Acero equivalent.
...
These mappings also exist for slightly more complex functions which have additional parameters to pass in.
...
The output here doesn't show the function mapping as aggregation functions are a special case which are handled differently.

However, you can see the mapping as part of a more complex output - if you call `show_query()` as a final step.
...
However, there are some niche use-cases where this could be helpful - for example, if you want to use Acero options which don't exist in the R equivalent function, or there is equivalent R function at all.
...
For the most part, Acero functions have an R mapping, which are generally simpler to work with, and in general, we recommend using those unless you have a specific reason to want to call the Acero equivalent directly like in the example above.
