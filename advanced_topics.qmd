...
| Feature                        | R (`sd`)                                                | Acero (`stddev`)                                                                        |
|-------------------|----------------------|--------------------------------|
| **Function Name**              | `sd`                                                    | `stddev`                                                                                |
| **Function Options**           | `na.rm = FALSE`                                         | `ddof = 0`, `skip_nulls = FALSE`, `min_count = 1`                                       |
| **Purpose**                    | Computes standard deviation                             | Computes standard deviation                                                             |
| **Handling of Missing Values** | `na.rm`: Boolean, removes `NA` values (default `FALSE`) | `skip_nulls`: Boolean, removes `NULL` values (default `FALSE`)                          |
| **Degrees of Freedom**         | Not available                                           | `ddof`: Integer, denominator degrees of freedom (default `0`)                           |
| **Minimum Count**              | Not available                                           | `min_count`: Integer, returns `NULL` if fewer non-null values are present (default `1`) |
...
The table below shows a comparison with the Acero equivalent.

| Feature                     | R (`startsWith()`)                                | Acero (`starts_with()`)                            |
|-----------------|------------------------|--------------------------------|
| **Function Name**           | `startsWith`                                      | `starts_with`                                      |
| **Function Arguments**      | `prefix`                                     | `pattern`, `ignore_case = false`              |
| **Purpose**                 | Checks if a string starts with a specified prefix | Checks if a string starts with a specified pattern |
| **Prefix/Pattern Argument** | `prefix`: The prefix to match                     | `pattern`: The pattern to match                    |
| **Case Sensitivity**        | Always case-sensitive                             | `ignore_case`: Boolean, set to `false` by default  |

There is already a mapping between these two functions, and we can see this by creating our query but not actually executing it.
...
You can also use these functions in your arrow dplyr pipelines by prefixing them with `"arrow_"`.
...
However, there are some niche use-cases where this could be helpful - for example, if you want to use Acero options which don't exist in the R equivalent function.

Let's take a look at how we'd do that with the Acero function `starts_with()`, if we wanted the returned results to now be case-insensitive.
...
open_dataset("data/person/") |> 
  ...
  filter(
    arrow_starts_with(
      location,
      options=list(pattern="w", ignore_case = TRUE)
    )
  )
```

For the most part, Acero functions have an R mapping, which are generally simpler to work with, and in general, we recommend using those unless you have a specific reason to want to call the Acero equivalent directly.
...
![How Acero bindings work](images/excalidraw/adv_normal.png) The extra steps needed are because we are working more directly with Acero, the part of the Arrow C++ library that runs the data manipulation queries, and as Acero is written in C++ we have to make sure we are giving it the correct information to work with.
...
We start off with an Arrow Dataset, which is turned into a dataset query when we call `summarize()`.
We're still using lazy evaluation here, and no calculations have been run on our data.
...
But there is no technical limitations preventing duckdb from improving dplyr support---and by the time you are reading this, they very well may have\![
\^duckplyr\]---nor from there being a SQL interface to arrow.
...
It also sets the bounding box and center so that the entirety of the United States is laid out correctly.[^advanced_topics-1]
Finally, we pull the data into R and set the correct geometry column to use.

[^advanced_topics-1]: Without this, some of the Aleutian islands which are to the west of the 180th meridian will be plotted to the east of the rest of the United States.
...
Putting this all together, let's try making a plot[^advanced_topics-2] showing different languages spoken throughout the United States.
...
In the PUMS dataset, the smallest geographic area is called a [PUMA (Public Use Microdata Areas)](https://www.census.gov/programs-surveys/geography/guidance/geo-areas/pumas.html).
[^advanced_topics-3] These PUMAs are designed to have approximately 100,000 people in them, and must be within a single state
.

[^advanced_topics-2]: This type of plot is called a choropleth where a geographic representation is colored or shaded to show different values in different areas.

[^advanced_topics-3]: There is also a more [detailed description of PUMAs](https://walker-data.com/census-r/introduction-to-census-microdata.html#what-is-a-puma) in Kyle Walker's *Analyzing US Census Data: Methods, Maps, and Models in R*
...
And then we can take that data, join it to our arrow table of PUMAs and plot it using `ggplot2`.[^advanced_topics-4]
We can clearly see some patterns of migration: French is spoken in parts of northern Maine and Louisiana.

[^advanced_topics-4]: You may notice a function `fill_pumas()` in the code below.
