...
open_dataset("data/person/") |>
  ...
  transmute(
    age = AGEP,
    self_employed = stringr::str_detect(as.character(COW), "Self-employed"),
    commute_time_predicted = predict_commute_time(age, self_employed)
  ) |>
  collect() |>
  head()
```
...
You'll also be slowed down a little by the fact that arrow has to convert back and forth between R and arrow, and R is single-threaded, so each calculation must be run one at a time instead of allowing parallelization.
