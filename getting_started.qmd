...
First, we'll create a couple of functions we can use in our pipeline.

```{r}
#| label: pums-data-manip-agg-prep
get_mode_of_transit <- function(mode_of_transit) {
  case_when(
...
get_public_private <- function(mode_of_transit) {
  
  private_transport <- c("Car, motorcycle", "Bicycle", "Walked")
  public_transport <- c("Bus", "Subway, light rail", 
                        "Commuter rail, train")
...
commute_by_mode <- pums_person |>
  mutate(
    ...
    mode_of_transit = get_mode_of_transit(mode_of_transit),
    public_private = get_public_private(mode_of_transit)
  ) |>
  filter(!is.na(mode_of_transit)) |>
  group_by(year, mode_of_transit, public_private) |>
  summarize(
    mean_commute_time = sum(JWMNP * PWGTP,
                            na.rm = TRUE) / sum(PWGTP),
...
to_plot |>
  ...
  geom_rect(
    mapping = aes(xmin = latest_year + .1, xmax = Inf, 
                  ymin = -Inf, ymax = Inf),
