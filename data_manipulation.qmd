...
pums_person |>
  mutate(
    CIT = case_when(
      CIT == "Yes, born in the US" ~ "Born in the United States",
      CIT == "Yes, naturalized" ~ "U.S. citizen by naturalization",
      CIT == "Not a citizen" ~ "Not a U.S. citizen",
      CIT == "Yes, born abroad of American parent(s)" ~ "Born abroad of U.S. citizen parent or parents",
      CIT == "Yes, born in Puerto Rico, etc." ~ "Born in Puerto Rico, Guam, the U.S. Virgin Islands, or Northern Marianas"
    )
  ) |>
  distinct(CIT) |>
  collect()
...
pums_person |>
  ...
  mutate(self_employed = stringr::str_detect(COW, "Self-employed")) |>
...
higher_ed <- pums_person |>
  filter(AGEP > 18) |>
  mutate(higher_education = grepl("(Bach|Mast|Prof|Doct|college|degree)", SCHL))
```
...
higher_ed <- pums_person |>
  filter(AGEP > 18) |>
  mutate(higher_education = stringr::str_detect(SCHL, "(Bach|Mast|Prof|Doct|college|degree)"))
```
...
mean_commute_time <- pums_person |>
  ...
  summarize(mean_commute_time = sum(JWMNP * PWGTP, na.rm = TRUE) / sum(PWGTP)) |>
  collect()
...
![Joining data](images/excalidraw/data_manip_join.png) The code to accomplish this looks something like this:
...
pums_person |>
  ...
  select(commute_diff, commute_time = JWMNP, mean_commute_time) |>
  head(3) |>
  collect()
```
...
We can manually set the schema of the table by converting it to an Arrow Table and setting the schema at the same time.
...
pums_person |>
  mutate(year = cast(year, int16())) |>
  left_join(mean_commute_time_arrow, by = c("location", "year"))|>
  mutate(commute_diff = mean_commute_time - JWMNP) |>
  select(commute_diff, commute_time = JWMNP, mean_commute_time) |>
  head(3) |>
  collect()
