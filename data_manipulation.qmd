...
In the example earlier, we called `collect()` to pull our `mean_commute_time` reference table into our R session, and so it is a tibble.
When we perform joins to data.frame or tibble objects, arrow converts these objects into Arrow Tables, with the data type of each column automatically being converted into its Arrow equivalent.
For the most part, this automatic conversion is fine, but in some circumstances like joining tables, we might want to manually specify the data types to convert to instead.
