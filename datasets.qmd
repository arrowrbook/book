...
![Dataset components](images/excalidraw/datasets_dataset.png){width="1250"}
...
open_dataset("./data/person") |>
  filter(location == "wa") |>
  group_by(year) |>
  summarize(avg_commute = sum(JWMNP * PWGTP, na.rm = TRUE) / sum(PWGTP)) |>
  arrange(desc(year)) |>
  collect()
```
...
![](images/excalidraw/datasets_pums.png)
...
#| label: save-non-hive
open_dataset("./data/person") |>
  write_dataset("./data/transient_data/person_non_hive", partitioning = "year", hive_style = FALSE)
```
...
```{r}
#| label: non-hive-size
fs::dir_ls("./data/transient_data/person_non_hive")
```
...
```{r}
#| label: hive-no-year
ds <- open_dataset("./data/transient_data/person_non_hive")
```
...
ds <- open_dataset("./data/transient_data/person_non_hive", partitioning = "year")
```
...
ds <- open_dataset("./data/transient_data/person_non_hive", partitioning = hive_partition(year = int16()))
```
...
open_dataset("./data/person") |>
  group_by(year) |>
  write_dataset("./data/transient_data/person_year")
```
...
open_dataset("./data/person") |>
  ...
  write_dataset(
    path = "./data/transient_data/person-age-partitions"
  )
```
...
open_dataset("./data/person") |>
  filter(year >= 2020) |>
  write_dataset(
    "./data/transient_data/person_starting_2020_fn",
...
```{r}
#| label: basename-view
fs::dir_ls("./data/transient_data/person_starting_2020_fn", recurse = TRUE)
```
...
open_dataset("./data/person") |>
  filter(year >= 2020) |>
  write_dataset(
    "./data/transient_data/person_starting_2020",
...
fs::dir_ls("./data/transient_data/logs")
```
...
#| echo: false
fs::dir_ls("./data/transient_data/logs_repartitioned")
```
...
partition_with_na_path <- "./data/transient_data/na-partition-default"
...
partition_with_custom_na_path <- "./data/transient_data/na-partition-custom"
...
open_dataset("./data/person") |>
  write_dataset("./data/transient_data/person_onefile")

pums_one_big_file <- open_dataset("./data/transient_data/person_onefile")
# 2. Partitioned by year
open_dataset("./data/person") |>
  write_dataset("./data/transient_data/person_year", partitioning = "year")

pums_by_year <- open_dataset("./data/transient_data/person_year")

# 3. Partitioned by year and location
# We don't need to create a new dataset here as this already is the structure of the data
...
open_dataset("./data/person") |>
  ...
  write_dataset(
    path = "./data/transient_data/person_year_location_age",
    partitioning = c("year", "location", "age_group")
  )

pums_by_year_location_age <- open_dataset("./data/transient_data/person_year_location_age")
```
...
+---------------------------+-------------+-----------+--------------+-------------+--------------+-----------+
| pums_by_year              | 34          | 0.1MB     | 0.1MB        | 211.8MB     | 459.9 MB     | 493MB     |
+---------------------------+-------------+-----------+--------------+-------------+--------------+-----------+
| pums_by_year_location     | 1785        | 0.1MB     | 0.1MB        | 0.1MB       | 6.02MB       | 56.1MB    |
+---------------------------+-------------+-----------+--------------+-------------+--------------+-----------+
| pums_by_year_location_age | 14124       | 0.1MB     | 0.1MB        | 0.4MB       | 1.09MB       | 17.6MB    |
+---------------------------+-------------+-----------+--------------+-------------+--------------+-----------+

```{r}
#| eval: false
#| echo: false
# This isn't run, I just included it here as it is how I got the values for the table
dir <- "./data/person"
...
median(fs::file_size(fs::dir_ls(dir, recurse = TRUE))) |> utils:::format.object_size("KB", digits = 2L)
...
open_dataset("<path/to/data>") |>
  filter(year >= 2018) |>
  summarise(mean_commute = sum(JWMNP * PWGTP, na.rm = TRUE) / sum(PWGTP)) |>
  collect()
```
...
open_dataset("<path/to/data>") |>
  ...
  filter(year >= 2012, location %in% c("ny", "ca"), age_group == "Under 25") |>
...
open_dataset("./data/person") |>
  filter(year >= 2018) |>
  group_by(year) |>
  write_dataset("./data/transient_data/person_starting_2018", max_rows_per_file = 500000)
```
...
fs::dir_ls("./data/transient_data/person_starting_2018", recurse = TRUE)
```
...
ds <- open_dataset(
  sources = c(
    file.path("data", "person", "year=2021", "location=ak", "part-0.parquet"),
...
#| echo: false
new_dir <- "./data/transient_data/person_with_metadata"
...
purrr::walk(
  ...
  ~ writeLines("PUMS data - this be metadata", file.path(.x, "data_info.txt"))
)
```
...
fs::dir_ls("./data/transient_data/person_with_metadata", recurse = TRUE)
```
...
#| label: metadata-fail
#| error: true
open_dataset("./data/transient_data/person_with_metadata")
```
...
#| eval: false
open_dataset(
  "./data/transient_data/person_with_metadata",
...
#| echo: false
fs::dir_delete("./data/transient_data/person_with_metadata")
```
...
#| echo: false
new_dir <- "./data/transient_data/person_multiformat"
...
parquet_dataset <- open_dataset("./data/transient_data/person_multiformat/parquet_data")
csv_dataset <- open_csv_dataset(
  "./data/transient_data/person_multiformat/csv_data",
  skip = 1,
...
combined_data |>
  group_by(year) |>
  summarise(mean_age = sum(JWMNP * PWGTP, na.rm = TRUE) / sum(PWGTP)) |>
  collect()
```
